...
from .knowledge_base import INSURANCE_KNOWLEDGE
...
class SimpleRAG:
    ...
    def get_answer(self, question: str) -> Dict[str, Any]:
        ...
        # Normalize question
        normalize_question = question.lower().strip()

        # Find the most relevant knowledge entry
        best_entry, score = self._retrieve_best_match(normalize_question)
    ...
    def _retrieve_best_match(self, question: str) -> Tuple[Dict[str, str], float]:
        """
        Retrieve the best matching entry from the knowledg base.
        ...
        for entry in self.knowledge_base:
            ...
            for keyword in keywords:
                if keyword in entry_question:
                    score += 2                          # Higher weight for matches in question
                if keyword in entry_answer:
                    score += 1                          # Lower weight for matches in the answer

            # Normalize the score based on number of keywords
            if keywords:
        ...
        # If there's no good match, return a default response
        ...
        return best_entry, min(best_score, 1.0)         # Cap the score at 1.0
    ...
    def _extract_keywords(self, text: str) -> List(str):
        ...
        insurance_terms = ["water damage", "flood insurance", "home insurance", "homeowners",
                          "policy", "claim", "deductible", "coverage", "premium", "adjuster",
                          "actual cash value", "replacement cost", "liability", "property damage",
                          "personal property", "dwelling", "endorsement", "exclusion", "peril",
                          "subrogation", "underwriting", "loss", "damage", "settlement"]
